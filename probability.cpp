...
std::size_t total_scenaries_count = 0;

const std::map<int, std::vector<int>> treasure_values = {{1, {0, 1, 2, 3}},
                                                         {2, {4, 5, 6, 7}},
                                                         {3, {8, 9, 10, 11}},
                                                         {4, {11, 12, 13, 14}}};


const std::map<int, double> dice_probabilities = {{2, 1.0 / 9.0},
                                                  {3, 2.0 / 9.0},
                                                  {4, 3.0 / 9.0},
                                                  {5, 2.0 / 9.0},
                                                  {6, 1.0 / 9.0}};
...
void calculate_final_win_prob(GameState state, double final_prob) {
  ...
  std::function<void(int, int)> generate_red_sums = [&](int treasure_idx,
                                                        int current_sum) {
  ...
  std::function<void(int, int)> generate_green_sums = [&](int treasure_idx,
                                                          int current_sum) {
  ...
  if (state.red_treasures.empty())
    red_values_to_sum.push_back(0);

  generate_green_sums(0, 0);
  if (state.green_treasures.empty())
    green_values_to_sum.push_back(0);
  ...
  long long total_combinations =
      red_values_to_sum.size() * green_values_to_sum.size();
  if (total_combinations > 0) {
    red_win_probability +=
        final_prob *
        (static_cast<double>(red_winning_combinations) / total_combinations);
  }
}
...
void simulate_turn(GameState state, bool is_red_turn, double current_prob) {
  ...
  auto handle_next_step = [&](GameState resulting_state,
                              double probability_of_this_path) {
  ...
  bool &current_player_is_back =
      is_red_turn ? state.red_is_back : state.green_is_back;
  ...
  std::vector<int> &current_treasures =
      is_red_turn ? state.red_treasures : state.green_treasures;
  ...
  for (const auto &pair : dice_probabilities) {
    ...
    int &next_player_pos =
        is_red_turn ? next_state.red_pos : next_state.green_pos;
    next_player_pos = new_pos;

    if (new_pos <= 0) {
      bool &is_back_flag =
          is_red_turn ? next_state.red_is_back : next_state.green_is_back;
      is_back_flag = true;
      handle_next_step(next_state, current_prob * dice_prob);
    } else {
      if (next_state.board.count(new_pos) &&
          next_state.board.at(new_pos) > 0) {
        GameState state_after_taking = next_state;
        std::vector<int> &treasures = is_red_turn
                                          ? state_after_taking.red_treasures
                                          : state_after_taking.green_treasures;
        ...
        std::vector<int> &treasures_to_drop_from =
            is_red_turn ? next_state.red_treasures : next_state.green_treasures;
        if (!treasures_to_drop_from.empty()) {
          GameState state_after_dropping = next_state;
          std::vector<int> &treasures =
              is_red_turn ? state_after_dropping.red_treasures
                          : state_after_dropping.green_treasures;
          auto min_it = std::min_element(treasures.begin(), treasures.end());
          state_after_dropping.board[new_pos] = *min_it;
          treasures.erase(min_it);
          handle_next_step(state_after_dropping,
                           current_prob * dice_prob * 0.5);
...
int main() {
  ...
  initial_state.board = {{1, 1}, {2, 0}, {3, 1}, {4, 1}, {5, 1}};
  ...
  std::cout << "------------------------------------------" << std::endl;
  std::cout << "Total Red Player Win Probability: " << red_win_probability
            << std::endl;
  std::cout << "------------------------------------------" << std::endl;
  std::cout << "Total scenaries count: " << total_scenaries_count << std::endl;
  std::cout << "------------------------------------------" << std::endl;
